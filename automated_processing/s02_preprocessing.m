...
%       MODIFIED ON:	2017_09_20 - 2018_01_09
%
...
function []=s02_preprocessing(dirname)
    ...
    sx = strcat('xm', anatfile.name);

    matlabbatch{1}.spm.util.imcalc.input = cellstr([manat_file; c1; c2; c3]);
    ...
    spm_jobman('interactive', matlabbatch);
    %spm_jobman('run', matlabbatch);
    fprintf('Skull stripping complete!');
    clear matlabbatch;

    % generate func file names
    funcfiles = {catstr('/', funcfiles.name)};
